...
from models import CarModel, UpdateCarModel, CarCollectionPagination, CARS_PER_PAGE
...
# @router.post(
#     '/',
#     response_description='Adding new car',
...
@router.post('/', response_description='Add new car with picture', response_model=CarModel,
             status_code=status.HTTP_201_CREATED)
...
@router.get('/', response_description="List all cars",
            response_model=CarCollectionPagination, response_model_by_alias=False )
async def list_cars(request: Request, page: int = 1, limit: int = CARS_PER_PAGE):

    cars = request.app.db['cars']
    results = []

    # total_documents = await 
    ...
    return CarCollectionPagination(cars=results, has_more=has_more)

@router.get('/{id}', response_description='Get a single car by Id',
            response_model=CarModel, response_model_by_alias=False)
...
@router.put('/{id}', response_description='Update or Delete Car',
             response_model= UpdateCarModel, response_model_by_alias=False)
async def update_car(id: str, request: Request, 
